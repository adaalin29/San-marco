<?php
    $tipPastePene = $detaliu_paste['optionals'][0]['optionalsContent'][0];
    $tipPasteSpa = $detaliu_paste['optionals'][0]['optionalsContent'][1];
...
<div class="container page-container">
    <div class="breadcrumb-container">
        ...
        <div class="breadcrumb-line">|</div>
        <a href="/detaliu-paste/{{$detaliu_paste['productId']}}" class="breadcrumb-element">{{str_replace(" XXL", "",$detaliu_paste['name'])}}</a>
    </div>
    
    <div class = "pizza-container">
        <div class = "pizza-left">
            <img
            src = "{{isset($detaliu_paste['image']) ? Voyager::image($detaliu_paste['image']) : 'images/pizza.png'}}" class = "full-width-2 object-cover">
        </div>
        <div class = "pizza-right">
            <div class = "pizza-title-container">
                <div class = "pizza-title">{{str_replace(" XXL", "", $detaliu_paste['name'])}}</div>
                <div class = "pizza-pret-container">
                    <div id="pizza-pret" class  = "pizza-pret">{{number_format($detaliu_paste['price'],2)}}</div>
            ...
            <div class = "ingrediente-container">
                ...
                @if(isset($detaliu_paste['ingredienteBlocate']) && count($detaliu_paste['ingredienteBlocate']) > 0)
                @foreach($detaliu_paste['removableToppings'] as $ingredient)
                <div class = "ingredient">
                    @if(!in_array($ingredient['toppingId'], $detaliu_paste['ingredienteBlocate']))
            ...
            <div class = "ingrediente-title">Alege tipul de paste:</div>
            <div class = "dimensiune-container">
            <div id="pene" class = "dimensiune-item" onclick="selecteazaTipul('{{json_encode($tipPastePene)}}', 'pene')">
                        <div class = "dimensiune-img" ><img src = "images/penne.svg" class  ="full-width"></div>
                        <div class = "dimensiune-check-container">
                            
                            <div class = "ingredient-text">Pene</div>
                        </div>
                    </div>

                    <div id="spaghetti" class = "dimensiune-item" onclick="selecteazaTipul('{{json_encode($tipPasteSpa)}}', 'spaghetti')">
                            <div class = "dimensiune-img"><img src = "images/spaghetti.svg" class  ="full-width"></div>
                            <div class = "dimensiune-check-container">
                                
                                <div class = "ingredient-text">Spaghetti</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div id='topping-container' class = "topping-container">
                        <div class="swiper-container">
                            <div class="swiper-wrapper">
                                @if(isset($detaliu_paste['toppinguriBlocate']) && count($detaliu_paste['toppinguriBlocate']) > 0)
                                @foreach($extraToppingsPaste as $extra)
                                @if(!in_array($extra['productId'], $detaliu_paste['toppinguriBlocate']))
                            ...
                            <div class="despre-buton adauga-produs" onclick="adaugaInCos('{{$detaliu_paste['name']}}', '{{$detaliu_paste['productId']}}', '{{json_encode($detaliu_paste)}}')">
                ...
                <script>
                    ...
                    function selecteazaTipul(data, tip){
                        $('#spaghetti').removeClass('topping-cos-checked')
                        $('#pene').removeClass('topping-cos-checked')
                            ...
                            html+=            '<div class="sos-tag-wrapper">'
                                ...
                                $('#topping-tag').append(html);
                                    ...
                                    function adaugaInCos(name, id, data){
                                        ...
                                        if(!selected){
                                            return Notiflix.Notify.Failure("Alege tipul de paste");
                                        }
