                ...
                <div class="panel panel-bordered">
                    ...
                    <form role="form"
                        ...
                        <div class="panel-body">
                            ...
                            <label class="control-label" for="name">Salata</label>
                            <input id='salata-name' type="hidden" name="salataname" @if(isset($selectedSalataName)) value="{{$selectedSalataName}}" @else value="" @endif>
                            @if(isset($ingrediente))
                            <input id='selected-ingrediente' type="hidden" value='{{json_encode($ingrediente)}}'>
                            @endif
                            <select required id="salata" name="salata" class="form-control select2">
                                <option value="">None</option>
                                
                                @foreach($salate['products'] as $salata)
                                @if(isset($selectedSalata))
                                @if($salata['productId'] == $selectedSalata)
                                <option selected value="{{$salata['productId']}}" data-ingrediente="{{json_encode($salata['removableToppings'])}}">{{$salata['name']}}</option>
                                @else
                                <option value="{{$salata['productId']}}" data-ingrediente="{{json_encode($salata['removableToppings'])}}">{{$salata['name']}}</option>
                                @endif
                                @else
                                <option value="{{$salata['productId']}}" data-ingrediente="{{json_encode($salata['removableToppings'])}}">{{$salata['name']}}</option>
                                @endif
                            ...
                            <div class="form-group col-md-12">
                                ...
                                <?php
                                foreach($salate['extraToppings'] as $topping){
                                    $found = false;
                                    if($toppings){
...
@section('javascript')
    <script>
        $( "#salata" ).change(function() {
            $('.select2-selection__rendered').empty();
            $("#toppings option:selected").each(function(){
                $(this).removeAttr('selected')
            })
            $('#salata-name').val($( "#salata option:selected" ).text())
            var ingrediente = $( "#salata option:selected" ).data("ingrediente")
