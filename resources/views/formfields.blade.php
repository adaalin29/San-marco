@if(isset($options->relationship))
    ...
    @else
        <select class="form-control select2" name="{{ $row->field }}" multiple></select>
    @endif
@else
    ...
    <select class="form-control select2" name="{{ $row->field }}" multiple>
        <?php $default = (isset($options->default) && !isset($dataTypeContent->{$row->field})) ? $options->default : null; ?>
        @if(!isset($options->options))
            @foreach($options as $produs)
                <option value="{{ $produs['productId'] }}" @if($default == $produs['productId'] && $selected_value === NULL) selected="selected" @endif @if($selected_value == $produs['productId']) selected="selected" @endif>{{ $produs['name'] }}</option>
            @endforeach
        @endif
    </select>
@endif